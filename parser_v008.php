...
<style>
	...
	.listArtists {float:left; clear:left; padding-left:20px; padding-top:10px; font-size:12px;}
	...
	.listCH {float:left; padding-left:20px; padding-top:10px; font-size:12px;}
...
<?php
	...
	while($data = fgetcsv($handle, 30000, ",")){
		...
		$entries[] = array(
			...
			'yearlyRank' => $data[1],
			'prefix' => $data[3],
			'ch' => $data[4], //# of weeks on the charts
			'high' => $data[8],
			'artist' => $data[10],
			'artist-i' => $data[11],
			'featured' => $data[12],
...
<?php
	...
	if ($aggregate == "frequency") {
		echo "<p>Each list will display the top 10 highiest frequency entries of each category as well as the amount of times they appear on the top 100 Billboard charts from 1890 - 2011.</p>";
		...
		echo "<div class=\"listArtists\"><b>The top 10 artists on the Billboard Top 100</b><br />";
		...
		//echo "<a href=\"#moreArtists\">Click for more</a>";
		echo "</div>";
		
		//Calculate the top 10 albums
		for ($c = 1; $c <= 38317; $c++) {
		...
		echo "<div class=\"listAlbums\"><b>The top 10 albums on the Billboard Top 100</b><br />";
		...
		echo "<div class=\"listLabels\"><b>The top 10 labels on the Billboard Top 100</b><br />";
		...
		//Calculate the top 10 songs that have been on the charts the longest
		for ($c = 1; $c <= 38317; $c++) {
			$ch[$c] = $entries[$c]['ch'];
		}
		arsort($ch); //you can't use rsort() because it will change the keys into numbers, while arsort maintains the index/value association
		
		echo "<div class=\"listCH\"><b>The top 10 longest lasting songs on the top 100 billboard by weeks</b><br />";
		
		$num_ch = 1;
		foreach ($ch as $key => $value) {
			if ($num_ch < 11) {
				if ($key != "" & $value != "n/a") { //gets rid of blanks
					echo $num_ch . ") " . $entries[$key]['artist'] . " - " . $entries[$key]['title'] , " - <i>" . $value . " weeks</i><br />";
					$num_ch++;
				}
			}
		}
